...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.get(
            "/roles/:role",
            param("role").isString().isIn(["Manager", "Customer","Admin"]), //the request body must contain an attribute named "role", the attribute must be a string and its value must be one of the two allowed options
            this.errorHandler.validateRequest, //middleware defined in `helper.ts`, checks the result of all the evaluations performed above and returns a 422 error if at least one fails or continues if there are no issues
            this.authService.isAdmin,
        ...
        this.router.get(
            "/:username",
            param("username").isString().isLength({ min: 1 }), //the request body must contain an attribute named "username", the attribute must be a non-empty string
            this.errorHandler.validateRequest, //middleware defined in `helper.ts`, checks the result of all the evaluations performed above and returns a 422 error if at least one fails or continues if there are no issues
        ...
        this.router.patch(
            ...
            [
                ...
                check('birthdate').notEmpty().isDate({format: 'YYYY-MM-DD', strictMode: true}),
            ],
