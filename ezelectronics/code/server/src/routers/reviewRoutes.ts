...
class ReviewRoutes {
    ...
    initRoutes() {
        ...
        this.router.delete(
            ...
            validateProductModelMiddleware, // Middleware to validate request parameters
        ...
        function validateProductModelMiddleware(req: any, res: any, next: any) {
            const model = req.params.model;

            if (!model || typeof model !== 'string' || model.trim().length === 0) {
                return res.status(422).json({ message: 'Invalid product model: It must be a non-empty string' });
            }
            next();
        }
